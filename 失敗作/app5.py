...
def generate_detailed_prompt(genre, protagonist, length, traits, setting, challenge):
    prompt = f"ジャンルは{genre}で、主人公は{protagonist}です。物語は{length}で展開されます。"
    prompt += f"\n主人公は「{traits}」という性格や特性を持っており、物語の背景は{setting}です。"
...
traits = st.text_input("主人公の性格や特性を入力してください（例: 誠実、頭脳明晰、謎めいた）")
setting = st.text_input("物語の背景や設定を入力してください（例: 中世ファンタジーの世界、未来都市）")
challenge = st.text_input("主人公が直面する挑戦や困難を入力してください（例: 世界を救う使命、過去のトラウマ）")
...
if st.button('✨ ストーリーを生成'):
    if protagonist and traits and setting and challenge:
        # プロンプトを生成
        prompt = generate_detailed_prompt(genre, protagonist, length, traits, setting, challenge)
